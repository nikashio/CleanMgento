	...
	function phpString_(stream, state, closing, escapes) {
		...
		if (escapes !== false && stream.match(/^\$[a-zA-Z_][a-zA-Z0-9_]*/)) {
			...
			if (stream.match("[", false)) {
				...
				state.tokenize = matchSequence([[["[", null]], [[/\d[\w\.]*/, "number"], [/\$[a-zA-Z_][a-zA-Z0-9_]*/, "variable-2"], [/[\w\$]+/, "variable"]], [["]", null]]
				], closing, escapes);
			}
			if (stream.match(/\-\>\w/, false)) {
				// Match object operator
				state.tokenize = matchSequence([[["->", null]], [[/[\w]+/, "variable"]]
				], closing, escapes);
			}
			return "variable-2";
		}
	...
	CodeMirror.defineMode("php", function(config, parserConfig) {
		...
		return {
			startState: function() {
				...
				return {
					html: html, php: php, curMode: parserConfig.startOpen ? phpMode : htmlMode, curState: parserConfig.startOpen ? php : html, pending: null
				};
			},
			...
			copyState: function(state) {
				...
				return {
					html: htmlNew, php: phpNew, curMode: state.curMode, curState: cur, pending: state.pending
				};
			},
