...
namespace Snmportal\Pdfbase\FontLib\TrueType;

use Snmportal\Pdfbase\FontLib\AdobeFontMetrics;
use Snmportal\Pdfbase\FontLib\Font;
use Snmportal\Pdfbase\FontLib\BinaryStream;
use Snmportal\Pdfbase\FontLib\Table\Table;
use Snmportal\Pdfbase\FontLib\Table\DirectoryEntry;
use Snmportal\Pdfbase\FontLib\Table\Type\glyf;
use Snmportal\Pdfbase\FontLib\Table\Type\name;
use Snmportal\Pdfbase\FontLib\Table\Type\nameRecord;
...
class File extends BinaryStream
{
   ...
   */
    ...
    function getFontType()
    {
        ...
        return $class_parts[3]; // AUIT
        return $class_parts[1]; // AUIT
    }
    ...
    function parseTableEntries()
    {
        ...
        $class = "\\Snmportal\\Pdfbase\\FontLib\\$type\\TableDirectoryEntry";
    ...
    protected function readTable($tag)
    {
        ...
        if (!self::$raw) {
            ...
            $class = "\\Snmportal\\Pdfbase\\FontLib\\Table\\Type\\$name_canon";

            if (!isset($this->directory[$tag]) || !class_exists($class)) {
                return;
            }
        } else {
            $class = "\\Snmportal\\Pdfbase\\FontLib\\Table\\Table";
        }
