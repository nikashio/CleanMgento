 ...
 */
//AUIT
namespace Snmportal\Pdfbase\Dompdf\lib;

use Exception;
use Imagick;
use Magento\Framework\App\ObjectManager;
use Snmportal\Pdfbase\FontLib\Font;
use Snmportal\Pdfbase\FontLib\BinaryStream;
use Snmportal\Pdfbase\Svg\Document;
use Snmportal\Pdfbase\Svg\Surface\SurfaceCpdf;
...
//      }
    ...
    private function openFont($font)
    {
        ...
        if (file_exists($fontcache . '/' . $cache_name)) {
            ...
            $this->fonts[$font] = ObjectManager::getInstance()->get('\Magento\Framework\Serialize\Serializer\Json')->unserialize(file_get_contents($fontcache . '/' . $cache_name));
        ...
        if (!isset($this->fonts[$font]) && file_exists($dir . $metrics_name)) {
            ...
            if (is_dir($fontcache) && is_writable($fontcache)) {
                $tmp = ObjectManager::getInstance()->get('\Magento\Framework\Serialize\Serializer\Json')->serialize($data);
                file_put_contents($fontcache .'/'. $cache_name, $tmp);
    ...
    function stream($options = '')
    {
        ...
        if (headers_sent()) {
//AUIT            die("Unable to stream pdf: headers already sent");
        }
...
//AUIT        echo $tmp;
    ...
    function registerText($font, $text)
    {
        ...
        //AUIT
        $this->stringSubsetsText[$font][] = $text;
    }
    ...
    function serializeObject($id)
    {
        if (array_key_exists($id, $this->objects)) {
            return  ObjectManager::getInstance()->get('\Magento\Framework\Serialize\Serializer\Serialize')->serialize($this->objects[$id]);
    ...
    function restoreSerializedObject($obj)
    {
        ...
        $this->objects[$obj_id] = ObjectManager::getInstance()->get('\Magento\Framework\Serialize\Serializer\Serialize')->unserialize($obj);
        $this->closeObject();
...
//AUIT                print '[addImagePng ' . $file . ']';
            ...
            if ($data == '') {
                ...
                //debugpng
                if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                    print 'trouble writing file from GD';
                }
            }
    ...
    function addSvgFromFile($file, $x, $y, $w = 0, $h = 0)
    {
        ...
        if ($dimensions["width"] > 0 && $dimensions["height"] > 0) {
            ...
            $surface = new SurfaceCpdf($doc, $this);//AUIT
            $doc->render($surface);
        ...
        if (isset($this->imagelist[$file])) {
            ...
            if (!$error) {
                ...
                if (mb_substr($data, 0, 8, '8bit') != $header) {
                    ...
                    if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                        print '[addPngFromFile this file does not have a valid header ' . $file . ']';
                    }
                    ...
                    switch ($chunkType) {
                        case 'IHDR':
                            ...
                            if ($info['compressionMethod'] != 0) {
                                ...
                                //debugpng
                                if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                                    print '[addPngFromFile unsupported compression method ' . $file . ']';
                                }
                            ...
                            if ($info['filterMethod'] != 0) {
                                ...
                                //debugpng
                                if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                                    print '[addPngFromFile unsupported filter method ' . $file . ']';
                                }
                ...
                */
                                ...
                                default:
                                    if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                                        print '[addPngFromFile unsupported transparency type ' . $file . ']';
                                    }
                                    break;
                            }
                ...
                if (!$haveHeader) {
                    ...
                    //debugpng
                    if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                        print '[addPngFromFile information header is missing ' . $file . ']';
                    }
                ...
                if (isset($info['interlaceMethod']) && $info['interlaceMethod']) {
                    ...
                    //debugpng
                    if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                        print '[addPngFromFile no support for interlaced images in pdf ' . $file . ']';
                    }
            ...
            if (!$error && $info['bitDepth'] > 8) {
                ...
                //debugpng
                if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                    print '[addPngFromFile bit depth of 8 or less is supported ' . $file . ']';
                }
            ...
            if (!$error) {
                switch ($info['colorType']) {
                    ...
                    default:
                        ...
                        //debugpng
                        if (defined("DEBUGPNG") && DEBUGPNG) {
//AUIT                            print '[addPngFromFile alpha channel not supported: ' . $info['colorType'] . ' ' . $file . ']';
                        }
