    ...
    public static function build_url($protocol, $host, $base_path, $url)
    {
        ...
        if (!in_array(mb_strtolower($protocol), ["http://", "https://", "ftp://", "ftps://"])) {
            ...
            if ($url[0] !== '/' && (strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN' || ($url[0] !== '\\' && $url[1] !== ':'))) {
                ...
                if (!$base_path) {
                    $base_path = __DIR__ . '/../../../media';
                    if (defined("SNMDOMPDF_MEDIA_DIR")) {
                        $base_path=SNMDOMPDF_MEDIA_DIR;
                    }
                }
...
//AUIT            echo basename($arr[0]["file"]) . " (" . $arr[0]["line"] . "): " . $arr[1]["function"] . ": ";
            Helpers::pre_r($msg);
        }
    }
    ...
    public static function record_warnings($errno, $errstr, $errfile, $errline)
    {
        ...
        if (!($errno & (E_WARNING | E_NOTICE | E_USER_NOTICE | E_USER_WARNING | E_DEPRECATED ))) {
...
//AUIT            echo $errstr . "\n";
        ...
        //error_log("\n" . print_r("$errno, $errstr, $errfile, $errline", true), 3, 'auit.log');
    }
