...
namespace Snmportal\Pdfbase\Barcode;
...
class Datamatrix
{
     ...
     * @protected
     */
    protected $last_enc = SNM_SNM_ENC_ASCII;
     ...
     * @protected
     */
    protected $chset_id = [SNM_SNM_ENC_C40 => 'C40', SNM_SNM_ENC_TXT => 'TXT', SNM_SNM_ENC_X12 => 'X12'];
...
// -----------------------------------------------------------------------------
    ...
    public function __construct($code)
    {
        ...
        if ($params[11] < $nd) {
            ...
            // add padding
            if ($this->last_enc == SNM_SNM_ENC_EDF) {
                // switch to ASCII encoding
                $cw[] = 124;
                ++$nd;
            } elseif (($this->last_enc != SNM_SNM_ENC_ASCII) and ($this->last_enc != SNM_SNM_ENC_BASE256)) {
                // switch to ASCII encoding
                $cw[] = 254;
                ++$nd;
            }
    ...
    protected function isCharMode($chr, $mode)
    {
        $status = false;
        switch ($mode) {
            case SNM_SNM_ENC_ASCII: { // ASCII character 0 to 127
                    $status = (($chr >= 0) and ($chr <= 127));
                    break;
            }
            case SNM_SNM_ENC_C40: { // Upper-case alphanumeric
                    $status = (($chr == 32) or (($chr >= 48) and ($chr <= 57)) or (($chr >= 65) and ($chr <= 90)));
                    break;
            }
            case SNM_SNM_ENC_TXT: { // Lower-case alphanumeric
                    $status = (($chr == 32) or (($chr >= 48) and ($chr <= 57)) or (($chr >= 97) and ($chr <= 122)));
                    break;
            }
            case SNM_SNM_ENC_X12: { // ANSI X12
                    $status = (($chr == 13) or ($chr == 42) or ($chr == 62));
                    break;
            }
            case SNM_SNM_ENC_EDF: { // ASCII character 32 to 94
                    $status = (($chr >= 32) and ($chr <= 94));
                    break;
            }
            case SNM_SNM_ENC_BASE256: { // Function character (FNC1, Structured Append, Reader Program, or Code Page)
                    $status = (($chr == 232) or ($chr == 233) or ($chr == 234) or ($chr == 241));
                    break;
            }
            case SNM_SNM_ENC_ASCII_EXT: { // ASCII character 128 to 255
                    $status = (($chr >= 128) and ($chr <= 255));
                    break;
            }
            case SNM_SNM_ENC_ASCII_NUM: { // ASCII digits
    ...
    protected function lookAheadTest($data, $pos, $mode)
    {
        ...
        while (true) {
            ...
            if (($pos + $charscount) == $data_length) {
                if ($numch[SNM_SNM_ENC_ASCII] <= ceil(min($numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_EDF], $numch[SNM_SNM_ENC_BASE256]))) {
                    return SNM_SNM_ENC_ASCII;
                }
                if ($numch[SNM_SNM_ENC_BASE256] < ceil(min($numch[SNM_SNM_ENC_ASCII], $numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_EDF]))) {
                    return SNM_SNM_ENC_BASE256;
                }
                if ($numch[SNM_SNM_ENC_EDF] < ceil(min($numch[SNM_SNM_ENC_ASCII], $numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_BASE256]))) {
                    return SNM_SNM_ENC_EDF;
                }
                if ($numch[SNM_SNM_ENC_TXT] < ceil(min($numch[SNM_SNM_ENC_ASCII], $numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_EDF], $numch[SNM_SNM_ENC_BASE256]))) {
                    return SNM_SNM_ENC_TXT;
                }
                if ($numch[SNM_SNM_ENC_X12] < ceil(min($numch[SNM_SNM_ENC_ASCII], $numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_EDF], $numch[SNM_SNM_ENC_BASE256]))) {
                    return SNM_SNM_ENC_X12;
                }
                return SNM_SNM_ENC_C40;
            }
            // get char
            $chr = ord($data[($pos + $charscount)]);
            $charscount++;
            // STEP L
            if ($this->isCharMode($chr, SNM_SNM_ENC_ASCII_NUM)) {
                $numch[SNM_SNM_ENC_ASCII] += (1 / 2);
            } elseif ($this->isCharMode($chr, SNM_SNM_ENC_ASCII_EXT)) {
                $numch[SNM_SNM_ENC_ASCII] = ceil($numch[SNM_SNM_ENC_ASCII]);
                $numch[SNM_SNM_ENC_ASCII] += 2;
            } else {
                $numch[SNM_SNM_ENC_ASCII] = ceil($numch[SNM_SNM_ENC_ASCII]);
                $numch[SNM_SNM_ENC_ASCII] += 1;
            }
            // STEP M
            if ($this->isCharMode($chr, SNM_SNM_ENC_C40)) {
                $numch[SNM_SNM_ENC_C40] += (2 / 3);
            } elseif ($this->isCharMode($chr, SNM_SNM_ENC_ASCII_EXT)) {
                $numch[SNM_SNM_ENC_C40] += (8 / 3);
            } else {
                $numch[SNM_SNM_ENC_C40] += (4 / 3);
            }
            // STEP N
            if ($this->isCharMode($chr, SNM_SNM_ENC_TXT)) {
                $numch[SNM_SNM_ENC_TXT] += (2 / 3);
            } elseif ($this->isCharMode($chr, SNM_SNM_ENC_ASCII_EXT)) {
                $numch[SNM_SNM_ENC_TXT] += (8 / 3);
            } else {
                $numch[SNM_SNM_ENC_TXT] += (4 / 3);
            }
            // STEP O
            if ($this->isCharMode($chr, SNM_SNM_ENC_X12) or $this->isCharMode($chr, SNM_SNM_ENC_C40)) {
                $numch[SNM_SNM_ENC_X12] += (2 / 3);
            } elseif ($this->isCharMode($chr, SNM_SNM_ENC_ASCII_EXT)) {
                $numch[SNM_SNM_ENC_X12] += (13 / 3);
            } else {
                $numch[SNM_SNM_ENC_X12] += (10 / 3);
            }
            // STEP P
            if ($this->isCharMode($chr, SNM_SNM_ENC_EDF)) {
                $numch[SNM_SNM_ENC_EDF] += (3 / 4);
            } elseif ($this->isCharMode($chr, SNM_SNM_ENC_ASCII_EXT)) {
                $numch[SNM_SNM_ENC_EDF] += (17 / 4);
            } else {
                $numch[SNM_SNM_ENC_EDF] += (13 / 4);
            }
            // STEP Q
            if ($this->isCharMode($chr, SNM_SNM_ENC_BASE256)) {
                $numch[SNM_SNM_ENC_BASE256] += 4;
            } else {
                $numch[SNM_SNM_ENC_BASE256] += 1;
            }
            // STEP R
            if ($charscount >= 4) {
                if (($numch[SNM_SNM_ENC_ASCII] + 1) <= min($numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_EDF], $numch[SNM_SNM_ENC_BASE256])) {
                    return SNM_SNM_ENC_ASCII;
                }
                if ((($numch[SNM_SNM_ENC_BASE256] + 1) <= $numch[SNM_SNM_ENC_ASCII])
                        or (($numch[SNM_SNM_ENC_BASE256] + 1) < min($numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_EDF]))) {
                    return SNM_SNM_ENC_BASE256;
                }
                if (($numch[SNM_SNM_ENC_EDF] + 1) < min($numch[SNM_SNM_ENC_ASCII], $numch[SNM_SNM_ENC_C40], $numch[SNM_SNM_ENC_TXT], $numch[SNM_SNM_ENC_X12], $numch[SNM_SNM_ENC_BASE256])) {
                    return SNM_SNM_ENC_EDF;
                }
                if (($numch[SNM_ENC_TXT] + 1) < min($numch[SNM_ENC_ASCII], $numch[SNM_ENC_C40], $numch[SNM_ENC_X12], $numch[SNM_ENC_EDF], $numch[SNM_ENC_BASE256])) {
                    return SNM_ENC_TXT;
                }
                if (($numch[SNM_ENC_X12] + 1) < min($numch[SNM_ENC_ASCII], $numch[SNM_ENC_C40], $numch[SNM_ENC_TXT], $numch[SNM_ENC_EDF], $numch[SNM_ENC_BASE256])) {
                    return SNM_ENC_X12;
                }
                if (($numch[SNM_ENC_C40] + 1) < min($numch[SNM_ENC_ASCII], $numch[SNM_ENC_TXT], $numch[SNM_ENC_EDF], $numch[SNM_ENC_BASE256])) {
                    if ($numch[SNM_ENC_C40] < $numch[SNM_ENC_X12]) {
                        return SNM_ENC_C40;
                    }
                    if ($numch[SNM_ENC_C40] == $numch[SNM_ENC_X12]) {
                        $k = ($pos + $charscount + 1);
                        while ($k < $data_length) {
                            $tmpchr = ord($data[$k]);
                            if ($this->isCharMode($tmpchr, SNM_ENC_X12)) {
                                return SNM_ENC_X12;
                            } elseif (!($this->isCharMode($tmpchr, SNM_ENC_X12) or $this->isCharMode($tmpchr, SNM_ENC_C40))) {
                                break;
                            }
                            ++$k;
                        }
    ...
    {
        switch ($mode) {
            case SNM_ENC_ASCII: { // ASCII character 0 to 127
                    $cw = 254;
                    break;
            }
            case SNM_ENC_C40: { // Upper-case alphanumeric
                    $cw = 230;
                    break;
            }
            case SNM_ENC_TXT: { // Lower-case alphanumeric
                    $cw = 239;
                    break;
            }
            case SNM_ENC_X12: { // ANSI X12
                    $cw = 238;
                    break;
            }
            case SNM_ENC_EDF: { // ASCII character 32 to 94
                    $cw = 240;
                    break;
            }
            case SNM_ENC_BASE256: { // Function character (FNC1, Structured Append, Reader Program, or Code Page)
                    $cw = 231;
                    break;
            }
        }
    ...
    protected function getHighLevelEncoding($data)
    {
        ...
        $enc = SNM_ENC_ASCII; // current encoding mode
        ...
        while ($pos < $data_lenght) {
            switch ($enc) {
                case SNM_ENC_ASCII: { // STEP B. While in ASCII encodation
                    if (($data_lenght > 1) and ($pos < ($data_lenght - 1)) and ($this->isCharMode(ord($data[($pos)]), SNM_ENC_ASCII_NUM) and $this->isCharMode(ord($data[($pos + 1)]
                                                                                                                                                               ), SNM_ENC_ASCII_NUM))) {
                        ...
                        if ($newenc != $enc) {
                            ...
                            $chr = ord($data[($pos)]);
                            ++$pos;
                            if ($this->isCharMode($chr, SNM_ENC_ASCII_EXT)) {
                ...
                case SNM_ENC_X12 : { // ANSI X12
                    ...
                    do {
                        ...
                        if (isset($charset[$chr])) {
                            ...
                            } elseif (($enc == SNM_ENC_C40) and isset($this->chset['S3C'][$chr])) {
                                $temp_cw[] = 2; // shift 3
                                $shiftset = $this->chset['S3C'];
                            } elseif (($enc == SNM_ENC_TXT) and isset($this->chset['S3T'][$chr])) {
                                $temp_cw[] = 2; // shift 3
                ...
                case SNM_ENC_EDF: { // F. While in EDIFACT (EDF) encodation
                ...
                case SNM_ENC_BASE256: { // G. While in Base 256 (B256) encodation
                        // initialize temporary array with 0 lenght
                        $temp_cw = [];
                        $field_lenght = 0;
