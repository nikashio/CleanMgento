...
use DOMDocument;
use DOMNode;
use Snmportal\Pdfbase\Dompdf\Adapter\CPDF;
use DOMXPath;
use Snmportal\Pdfbase\Dompdf\Frame;
use Snmportal\Pdfbase\Dompdf\Frame\Factory;
use Snmportal\Pdfbase\Dompdf\Frame\FrameTree;

//use Snmportal\Pdfbase\Dompdf\HTML5_Tokenizer;
//use Snmportal\Pdfbase\Dompdf\HTML5_TreeBuilder;
use Snmportal\Pdfbase\Dompdf\Image\Cache;
use Snmportal\Pdfbase\Dompdf\Renderer\ListBullet;
use Snmportal\Pdfbase\Dompdf\Renderer;
use Snmportal\Pdfbase\Dompdf\Css\Stylesheet;
 ...
 * the {@link Snmportal\Pdfbase\Dompdf\Adapter\CPDF} class provides a simple, stateless interface to
 * PDF generation.)  PDFLib support has now also been added, via the {@link
 * Snmportal\Pdfbase\Dompdf\Adapter\PDFLib}.
 *
 *
 * @package dompdf
 */
class Dompdf
{
    ...
    public function loadHtml($str, $encoding = null)
    {
        ...
        set_error_handler(['\Snmportal\Pdfbase\Dompdf\Helpers', "record_warnings"]);
        ...
        if (0 && $this->options->isHtml5ParserEnabled()) {
            /*
    ...
    public function render()
    {
        ...
        if (is_array($basePageStyle->size)) {
            $this->setPaper([0, 0, $basePageStyle->size[0], $basePageStyle->size[1]]);
        }
     ...
     *
     * @param string $size 'letter', 'legal', 'A4', etc. {@link Snmportal\Pdfbase\Dompdf\Adapter\CPDF::$PAPER_SIZES}
    ...
    /**
     * Return the underlying Canvas instance (e.g. Snmportal\Pdfbase\Dompdf\Adapter\CPDF, Snmportal\Pdfbase\Dompdf\Adapter\GD)
     *
     * @return Canvas
     */
