...
//AUIT            print "<pre>\n";
            /*DEBUGCSS*/
//AUIT            printf("_specificity(): 0x%08x \"%s\"\n", ($a << 24) | ($b << 16) | ($c << 8) | ($d), $selector);
            /*DEBUGCSS*/
//AUIT            print "</pre>";
        }
...
//AUIT                        printf("specificity: 0x%08x\n", $spec);
                        foreach ($arr as $s) {
//AUIT                            print "[\n";
//AUIT                            $s->debug_print();
//AUIT                            print "]\n";
...
//AUIT                    print "]\n";
            ...
            if ($this->_dompdf->get_option('debugCss')) {
//AUIT                print "DomElementStyle:\n";
//AUIT                print "[\n";
//AUIT                $style->debug_print();
//AUIT                print "]\n";
//AUIT                print "/$debug_nodename]\n</pre>";
            }
...
//AUIT        if ($this->_dompdf->get_option('debugCss')) print '[_parse_properties';
        ...
        foreach ($properties as $prop) {
          ...
          $important = preg_match("/(.*?)!\s*important/",$prop,$match);
            ...
            $prop = trim($prop);
            */
//AUIT            if ($this->_dompdf->get_option('debugCss')) print '(';
            ...
            if ($prop === "") {
//AUIT                if ($this->_dompdf->get_option('debugCss')) print 'empty)';
                continue;
            }

            $i = mb_strpos($prop, ":");
            if ($i === false) {
//AUIT                if ($this->_dompdf->get_option('debugCss')) print 'novalue' . $prop . ')';
                continue;
            }

            $prop_name = rtrim(mb_strtolower(mb_substr($prop, 0, $i)));
            $value = ltrim(mb_substr($prop, $i + 1));
//AUIT            if ($this->_dompdf->get_option('debugCss')) print $prop_name . ':=' . $value . ($important ? '!IMPORTANT' : '') . ')';
...
//AUIT        if ($this->_dompdf->get_option('debugCss')) print '[_parse_sections';
        foreach ($sections as $sect) {
            $i = mb_strpos($sect, "{");

            $selectors = explode(",", mb_substr($sect, 0, $i));
//AUIT            if ($this->_dompdf->get_option('debugCss')) print '[section';
...
//AUIT                if ($this->_dompdf->get_option('debugCss')) print '#' . $selector . '#';
