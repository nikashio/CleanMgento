 ...
 *
 * Snmportal\Pdfbase\Dompdf\Adapter\CPDF provides a simple stateless interface to the stateful one
...
class CPDF implements Canvas
{
    ...
    function __construct($paper = "letter", $orientation = "portrait", Dompdf $dompdf)
    {
        ...
        $this->_pdf = new \Snmportal\Pdfbase\Dompdf\lib\Cpdf(
            $size,
            true,
...
//AUIT            if ($this->_dompdf->get_option("debugPng")) print '[__destruct unlink ' . $img . ']';
    ...
    protected function _convert_gif_bmp_to_png($image_url, $type)
    {
        ...
        if (!function_exists($func_name)) {
            if (!method_exists("\\Snmportal\\Pdfbase\\Dompdf\\Helpers", $func_name)) {
                throw new Exception("Function $func_name() not found.  Cannot convert $type image: $image_url.  Please install the image PHP extension.");
            }
            $func_name = "\\Snmportal\\Pdfbase\\Dompdf\\Helpers::" . $func_name;
        }
...
//AUIT        if ($debug_png) print "[image:$img|$width|$height|$type]";

        switch ($type) {
            case "jpeg":
//AUIT                if ($debug_png) print '!!!jpg!!!';
                $this->_pdf->addJpegFromFile($img, $x, $this->y($y) - $h, $w, $h);
                break;

            case "gif":
            case "bmp":
//AUIT                if ($debug_png) print '!!!bmp or gif!!!';
                // @todo use cache for BMP and GIF
                $img = $this->_convert_gif_bmp_to_png($img, $type);
            // no break
            case "png":
//AUIT                if ($debug_png) print '!!!png!!!';

                $this->_pdf->addPngFromFile($img, $x, $this->y($y) - $h, $w, $h);
                break;

            case "svg":
//AUIT                if ($debug_png) print '!!!SVG!!!';
                $this->_pdf->addSvgFromFile($img, $x, $this->y($y) - $h, $w, $h);
                break;

            default:
//AUIT                if ($debug_png) print '!!!unknown!!!';
                break;
        }
    }
