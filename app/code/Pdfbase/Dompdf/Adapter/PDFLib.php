...
namespace Snmportal\Pdfbase\Dompdf\Adapter;

use Snmportal\Pdfbase\Dompdf\Canvas;
use Snmportal\Pdfbase\Dompdf\Dompdf;
use Snmportal\Pdfbase\Dompdf\Helpers;
use Snmportal\Pdfbase\Dompdf\Exception;
use Snmportal\Pdfbase\Dompdf\FontMetrics;
use Snmportal\Pdfbase\Dompdf\Image\Cache;
use Snmportal\Pdfbase\Dompdf\PhpEvaluator;

/**
 * PDF rendering interface
 *
 * Snmportal\Pdfbase\Dompdf\Adapter\PDFLib provides a simple, stateless interface to the one
 * provided by PDFLib.
 *
...
class PDFLib implements Canvas
{
     ...
     * @var array;
     */
    public static $PAPER_SIZES = []; // Set to Snmportal\Pdfbase\Dompdf\Adapter\CPDF::$PAPER_SIZES below.
     ...
     * Class constructor
     *
     * @param mixed $paper The size of paper to use either a string (see {@link Snmportal\Pdfbase\Dompdf\Adapter\CPDF::$PAPER_SIZES}) or
    ...
    function arc($x1, $y1, $r1, $r2, $astart, $aend, $color, $width, $style = [])
    {
        $this->_set_line_style($width, "butt", "", $style);
        $this->_set_stroke_color($color);
    ...
    function text($x, $y, $text, $font, $size, $color = [0, 0, 0], $word_spacing = 0, $char_spacing = 0, $angle = 0)
    {
     ...
     */
    function page_text($x, $y, $text, $font, $size, $color = [0, 0, 0], $word_space = 0.0, $char_space = 0.0, $angle = 0.0)
    {
        $_t = "text";
    ...
    function stream($filename, $options = null)
    {
        ...
        //error_log("\n" . print_r("pdflib:stream no use", true), 3, 'var/log/snm-portal.log');
        if (self::$IN_MEMORY) {
//            echo $data;
...
//AUIT            while (!feof($fh))
//AUIT                echo fread($fh, $chunk);
            fclose($fh);

            //debugpng
//AUIT            if ($this->_dompdf->get_option("debugPng")) print '[pdflib stream unlink ' . $this->_file . ']';
    ...
    function output($options = null)
    {
        ...
        if (self::$IN_MEMORY) {
            ...
            //debugpng
//AUIT            if ($this->_dompdf->get_option("debugPng")) print '[pdflib output unlink ' . $this->_file . ']';
            if (!$this->_dompdf->get_option("debugKeepTemp")) {
