...
(function(mod) {
	...
	function getDimensions(cm) {
		...
		return {
			fixedPos: compensateForHScroll(d), gutterTotalWidth: d.gutters.offsetWidth, gutterLeft: left, gutterWidth: width, wrapperWidth: d.wrapper.clientWidth
		};
	}
	...
	function applyTextInput(cm, inserted, deleted, sel, origin) {
		...
		for (var i = sel.ranges.length - 1; i >= 0; i--) {
			...
			var changeEvent = {
				from: from, to: to, text: multiPaste ? multiPaste[i % multiPaste.length] : textLines, origin: origin || (paste ? "paste" : cm.state.cutIncoming ? "cut" : "+input")
			};
	...
	TextareaInput.prototype = copyObj({
		init: function(display) {
			...
			on(te, "compositionstart", function() {
				...
				input.composing = {
					start: start, range: cm.markText(start, cm.getCursor("to"), {className: "CodeMirror-composing"})
				};
			});
	...
	ContentEditableInput.prototype = copyObj({
		...
		onContextMenu: nothing, resetPosition: nothing,
	...
	function mapFromLineView(lineView, line, lineN) {
		...
		for (var i = 0; i < lineView.rest.length; i++) {
			if (lineNo(lineView.rest[i]) > lineN) {
				return {
					map: lineView.measure.maps[i], cache: lineView.measure.caches[i], before: true
				};
			}
		}
	}
	...
	function prepareMeasureForLine(cm, line) {
		...
		return {
			line: line, view: view, rect: null, map: info.map, cache: info.cache, before: info.before, hasHeights: false
		};
	}
	...
	function measureCharPrepared(cm, prepared, ch, bias, varHeight) {
		...
		return {
			left: found.left, right: found.right, top: varHeight ? found.rtop : found.top, bottom: varHeight ? found.rbottom : found.bottom
		};
	}
	...
	function maybeUpdateRectForZooming(measure, rect) {
		...
		return {
			left: rect.left * scaleX, right: rect.right * scaleX, top: rect.top * scaleY, bottom: rect.bottom * scaleY
		};
	}
	...
	function startOperation(cm) {
		cm.curOp = {
			cm                    : cm, viewChanged: false,      // Flag that indicates that lines might need to be redrawn
	...
	function registerEventHandlers(cm) {
		...
		on(d.scroller, "touchstart", function(e) {
			if (!signalDOMEvent(cm, e) && !isMouseLikeTouchEvent(e)) {
				...
				var now = +new Date;
				d.activeTouch = {
					start: now, moved: false, prev: now - prevTouch.end <= 300 ? prevTouch : null
				};
				if (e.touches.length == 1) {
			...
			var loadFile = function(file, i) {
				...
				reader.onload = operation(cm, function() {
					...
					if (++read == n) {
						...
						var change = {
							from: pos, to: pos, text: cm.doc.splitLines(text.join(cm.doc.lineSeparator())), origin: "paste"
						};
	...
	function filterChange(doc, change, update) {
		var obj = {
			canceled: false, from: change.from, to: change.to, text: change.text, origin: change.origin, cancel: function() {
				this.canceled = true;
			}
		};
		if (update) {
	...
	function makeChangeSingleDoc(doc, change, selAfter, spans) {
		...
		if (change.from.line < doc.first) {
			...
			shiftDoc(doc, shift);
			change = {
				from: Pos(doc.first, 0), to: Pos(change.to.line + shift, change.to.ch), text: [lst(change.text)], origin: change.origin
			};
		}
		var last = doc.lastLine();
		if (change.to.line > last) {
			change = {
				from: change.from, to: Pos(last, getLine(doc, last).text.length), text: [change.text[0]], origin: change.origin
			};
		}
	...
	function makeChangeSingleDocInEditor(cm, change, spans) {
		...
		if (changeHandler || changesHandler) {
			var obj = {
				from: from, to: to, text: change.text, removed: change.removed, origin: change.origin
			};
			if (changeHandler) {
	...
	CodeMirror.prototype = {
		constructor: CodeMirror, focus: function() {
			window.focus();
	...
	var commands = CodeMirror.commands = {
		selectAll             : function(cm) {
			...
			deleteNearSelection(cm, function(range) {
				return {
					from: Pos(range.from().line, 0), to: clipPos(cm.doc, Pos(range.to().line + 1, 0))
				};
			});
	...
	function takeToken(cm, pos, precise, asArray) {
		function getObj(copy) {
			return {
				start: stream.start, end: stream.pos, string: stream.current(), type: style || null, state: copy ? copyState(doc.mode, state) : state
			};
		}
	...
	function buildLineContent(cm, lineView) {
		...
		var builder = {
			pre: elt("pre", [content], "CodeMirror-line"), content: content, col: 0, pos: 0, cm: cm, splitSpaces: (ie || webkit) && cm.getOption("lineWrapping")
		};
		lineView.measure = {};
	...
	Doc.prototype = createObj(BranchChunk.prototype, {
		constructor: Doc, // Iterate over the document. Supports two forms -- with only one
		...
		getHistory   : function() {
			return {
				done: copyHistoryArray(this.history.done), undone: copyHistoryArray(this.history.undone)
			};
	...
	function addChangeToHistory(doc, change, selAfter, opId) {
		...
		if ((hist.lastOp == opId || hist.lastOrigin == change.origin && change.origin && ((change.origin.charAt(0) == "+" && doc.cm && hist.lastModTime > time - doc.cm.options.historyEventDelay) || change.origin.charAt(0) == "*")) && (cur = lastChangeEvent(hist, hist.lastOp == opId))) {
			...
			cur = {
				changes: [historyChangeFromChange(doc, change)], generation: hist.generation
			};
			hist.done.push(cur);
